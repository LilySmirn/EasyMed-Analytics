import mysql from "mysql2/promise";
import { loadSpecialities } from "./loadSpecialities.mjs";
import { loadDoctors } from "./loadDoctors.mjs";
import { loadServiceCatalog } from "./loadServiceCatalog.mjs";
import { loadVisits } from "./loadVisits.mjs";

const dbConfig = {
    host: 'localhost',
    user: 'root',
    password: 'rootpass',
    database: 'analytics',
};

async function run() {
    const conn = await mysql.createConnection(dbConfig);

    await loadSpecialities(conn, "./dbload/data/provided.xlsx");

    await loadDoctors(conn, "./dbload/data/provided.xlsx");

    await loadServiceCatalog(conn, "./dbload/data/");

    await loadVisits(conn, "./dbload/data/provided.xlsx");

    await conn.end();
}

run().catch(console.error);
